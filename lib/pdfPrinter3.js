...
module.exports = async (req, res) => {
    try {
        ...
        data.forEach((paket, index) => {
            ...
            const logoHeight = 25;
            doc.image(logoPath, xPos + padding, yPos + padding, { width: contentWidth, height: logoHeight });

            const textStartYY = yPos + padding + logoHeight + 5;
            doc.fontSize(8)
                .font('Arial')
                .text('BG DELIVERY RENT-A-CAR DOO', xPos + padding, textStartYY, { width: contentWidth, align: 'left', lineGap: -1 })
                .text('Adresa: Tošin Bunar 134B, 11070, Novi Beograd-Beograd', xPos + padding, textStartYY + 12, { width: contentWidth, align: 'left', lineGap: -1 })
                .text('MB: 21832081 | PIB: 113250833', xPos + padding, textStartYY + 24, { width: contentWidth, align: 'left', lineGap: -1 })
                .text('Tel: +381665121212 | Email: info@bgdelivery.rs', xPos + padding, textStartYY + 36, { width: contentWidth, align: 'left', lineGap: -1 });
            ...
            const bgEndY = textStartYY + 36 + doc.currentLineHeight();
            drawDivider(bgEndY + dividerPadding);
            let y = bgEndY + dividerPadding + 6;

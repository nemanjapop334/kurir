...
module.exports = async (req, res) => {
    try {
        if (!ensurePdfAccess(req, res, 'pdf1')) {
            return;
        }
        ...
        data.forEach((paket, index) => {
            ...
            doc.fontSize(8)
                .font('Arial')
                .text('BG DELIVERY RENT-A-CAR DOO', xPos + 10, textStartYY, { width: labelWidth - 20, align: 'left', lineGap: -1 }) // Manji razmak između linija
                .text('Adresa: Tošin Bunar 134B, 11070, Novi Beograd-Beograd', xPos + 10, textStartYY + 12, { width: labelWidth - 20, align: 'left', lineGap: -1 })
                .text('MB: 21832081 | PIB: 113250833', xPos + 10, textStartYY + 24, { width: labelWidth - 20, align: 'left', lineGap: -1 })
                .text('Tel: +381665121212 | Email: info@bgdelivery.rs', xPos + 10, textStartYY + 36, { width: labelWidth - 20, align: 'left', lineGap: -1 });
            ...
            doc.font('ArialBold')
                .fontSize(10)
                .text(`Ukupno: ${paket.cena + paket.ptt} RSD`, xPos + 10, textStartY + 98, { continued: true });

            doc.font('Arial')
                .fontSize(10)
                .text(`  Datum: ${paket.datum.toLocaleDateString('sr-RS')}`, { width: labelWidth - 20 });

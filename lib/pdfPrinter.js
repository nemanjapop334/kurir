...
module.exports = async (req, res) => {
    try {
        ...
        data.forEach((paket, index) => {
            ...
            const textStartY = yPos + logoHeight + 20; // Position text below the logo

            doc.fontSize(12)
                .font('ArialBold')
                .text(`Pošiljalac: ${paket.klijent}`, xPos + 10, textStartY, { width: labelWidth - 20, align: 'left' })
                .text(`Telefon: ${paket.telefon}`, xPos + 10, textStartY + 80, { width: labelWidth - 20 });

            doc.font('Arial')
                .fontSize(10)
                .text(`Grad: ${paket.grad || ''}`, xPos + 10, textStartY + 35, { width: labelWidth - 20 })
                .text(`Kontakt pošiljaoca: ${phone || ''}`, xPos + 10, textStartY + 20, { width: labelWidth - 20 })
                .text(`Adresa - Ime i prezime: ${paket.adresa}`, xPos + 10, textStartY + 50, { width: labelWidth - 20 })
                .text(`Ukupna cena: ${paket.cena + paket.ptt} RSD`, xPos + 10, textStartY + 95, { width: labelWidth - 20 })
                .text(`Datum: ${paket.datum.toLocaleDateString('sr-RS')}`, xPos + 10, textStartY + 110, { width: labelWidth - 20 });
            ...
            if (noteHeight > labelHeight - 135) {
                ...
                doc.text(shortenedText, xPos + 10, textStartY + 125, noteOptions);
            } else {
                doc.text(noteText, xPos + 10, textStartY + 125, noteOptions);
            }
        });
